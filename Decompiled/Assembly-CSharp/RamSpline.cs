...
public class RamSpline : MonoBehaviour
{
			...
			for (int l = 0; l < vertsInShape; l++)
			{
				...
				else
				{
					...
					if (Physics.Raycast(array[num9] + base.transform.position + Vector3.up * 5f, Vector3.down, out var hitInfo, 1000f, snapMask.value))
					{
						...
						if ((bool)terrain && !terrainsUnder.Contains(terrain))
						{
					...
					array[num9] += orientations[k] * Vector3.up * Mathf.Lerp(controlPointsMeshCurves[Mathf.FloorToInt(lerpValues[k])].Evaluate(num10), controlPointsMeshCurves[Mathf.CeilToInt(lerpValues[k])].Evaluate(num10), lerpValues[k] - Mathf.Floor(lerpValues[k]));
				}
				...
				foreach (RamSpline beginnigChildSpline2 in beginnigChildSplines)
				{
					if (!(beginnigChildSpline2 == null))
					{
						uv3length = length / fulllength + beginnigChildSpline2.uv3length;
						break;
					}
				}
				if (uvRotation)
	...
	public void ShowTerrainCarve(float differentSize = 0f, bool useCurve = true)
	{
		...
		int num = (int)terrainMeshSmoothZ;
		if (differentSize == 0f)
		{
		...
		for (int i = 0; i < carvePointsDown.Count - 1; i++)
		{
			...
			Vector3 vector = carvePointsDown[i];
			Vector3 vector2 = carvePointsUp[i];
			...
			if (terrainAdditionalWidth > 0f)
			{
				for (int j = 0; j < num; j++)
				{
					Vector3 vector4 = Vector3.Lerp(a, vector, (float)j / (float)num) + base.transform.position;
					if (Physics.Raycast(vector4 + Vector3.up * 500f, Vector3.down, out hitInfo, 10000f, maskCarve.value))
					{
						num2 = ((!noiseCarve) ? 0f : (Mathf.PerlinNoise(vector4.x * noiseSizeX, vector4.z * noiseSizeZ) * noiseMultiplierOutside - noiseMultiplierOutside * 0.5f));
						float num3 = 1f - (float)j / (float)num;
						num3 *= terrainAdditionalWidth;
						float b2 = vector4.y + terrainCarve.Evaluate(0f - num3) + terrainCarve.Evaluate(0f - num3) * num2;
						float f = (float)j / (float)num;
						f = Mathf.Pow(f, terrainSmoothMultiplier);
						b2 = (useCurve ? Mathf.Lerp(hitInfo.point.y, b2, f) : hitInfo.point.y);
						Vector4 item = new Vector4(hitInfo.point.x, b2, hitInfo.point.z, 0f - num3);
						list2.Add(item);
					}
					else
					{
			...
			for (int k = 0; k <= num; k++)
			{
				Vector3 vector4 = Vector3.Lerp(vector, vector2, (float)k / (float)num) + base.transform.position;
				if (Physics.Raycast(vector4 + Vector3.up * 500f, Vector3.down, out hitInfo, 10000f, maskCarve.value))
				{
					num2 = ((!noiseCarve) ? 0f : (Mathf.PerlinNoise(vector4.x * noiseSizeX, vector4.z * noiseSizeZ) * noiseMultiplierInside - noiseMultiplierInside * 0.5f));
					float num4 = magnitude * (0.5f - Mathf.Abs(0.5f - (float)k / (float)num));
					float b3 = vector4.y + terrainCarve.Evaluate(num4) + terrainCarve.Evaluate(num4) * num2;
					Mathf.Pow(1f - 2f * Mathf.Abs((float)k / (float)num - 0.5f), terrainSmoothMultiplier);
					b3 = (useCurve ? Mathf.Lerp(hitInfo.point.y, b3, 1f) : hitInfo.point.y);
					Vector4 item2 = new Vector4(hitInfo.point.x, b3, hitInfo.point.z, num4);
					list2.Add(item2);
				}
				else
				{
			...
			if (terrainAdditionalWidth > 0f)
			{
				for (int l = 1; l <= num; l++)
				{
					Vector3 vector4 = Vector3.Lerp(vector2, b, (float)l / (float)num) + base.transform.position;
					if (Physics.Raycast(vector4 + Vector3.up * 50f, Vector3.down, out hitInfo, 10000f, maskCarve.value))
					{
						num2 = ((!noiseCarve) ? 0f : (Mathf.PerlinNoise(vector4.x * noiseSizeX, vector4.z * noiseSizeZ) * noiseMultiplierOutside - noiseMultiplierOutside * 0.5f));
						float num5 = (float)l / (float)num;
						num5 *= terrainAdditionalWidth;
						float b4 = vector4.y + terrainCarve.Evaluate(0f - num5) + terrainCarve.Evaluate(0f - num5) * num2;
						float f2 = 1f - (float)l / (float)num;
						f2 = Mathf.Pow(f2, terrainSmoothMultiplier);
						b4 = (useCurve ? Mathf.Lerp(hitInfo.point.y, b4, f2) : hitInfo.point.y);
						Vector4 item3 = new Vector4(hitInfo.point.x, b4, hitInfo.point.z, 0f - num5);
						list2.Add(item3);
					}
					else
					{
				...
				for (int m = 0; m < heights.GetLength(0); m++)
				{
					for (int n = 0; n < heights.GetLength(1); n++)
					{
						...
						if (meshCollider.Raycast(ray, out var hitInfo, 10000f))
						{
							...
							if (flag)
							{
								Debug.DrawLine(hitInfo.point, hitInfo.point + Vector3.up * 0.1f, Color.magenta, 10f);
							}
						}
					}
				}
				terrainData.SetHeightsDelayLOD((int)num6, (int)num8, heights);
			}
	...
	public void TerrainPaintMeshBased(Terrain singleTerrain = null)
	{
		...
		foreach (Terrain terrain in activeTerrains)
		{
			...
			for (num4 = 0; num4 < pointsUp.Count; num4 = Mathf.Clamp(num4 + num3 - 1, 0, pointsUp.Count))
			{
				...
				if (alphamaps.GetLength(2) <= currentSplatMap)
				{
					...
					if (num7 - num6 > 0f && num9 - num8 > 0f)
					{
						for (int m = 0; m < terrainData.detailPrototypes.Length; m++)
						{
							...
							for (int n = 0; n < detailLayer.GetLength(0); n++)
							{
								...
								{
									zero.x = ((float)num10 + num6 + 0.5f) / num2 + terrain.transform.position.x;
									zero.z = ((float)n + num8 + 0.5f) / num + terrain.transform.position.z;
									Ray ray = new Ray(zero + Vector3.up * 3000f, Vector3.down);
